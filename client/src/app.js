...
function renderPanel() {
  ...
  if (tab === 'seal') {
    ...
    p.innerHTML = `<div class="note"><b>Bond USDG for $STYX at ${Bd ? fmt(Bd.discount * 100, 0) : 20}% below market.</b> Vests over ${Bd ? Bd.vestDays : 5} days. Your USDG goes to the reserve and mints nothing. ${Bd && !Bd.open ? '<b>Bonds are closed.</b>' : ''}</div>
      ...
      ${L ? `<div class="note" style="border-color:var(--gold)"><b>The Forge:</b> your USDG enters the bond pool, you take $STYX at <b>${F ? fmt(F.discount * 100, 0) : 30}% below market</b>, locked <b>${F ? F.lockDays * 24 : 48} hours</b>. While locked it earns <b>${F ? fmt(F.apy * 100, 0) : 80}% APY in $STYX</b>, paid from the Vigil's fixed pool. Nothing printed. Claim principal + yield at unlock.</div>` : ''}
    ...
    $('act').onclick = () => doAct('/api/bond', { amount: +$('in').value, lock: L }, (r) => r.lock ? `forged ${fmt(r.bonded, 2)} USDG → ${big(r.styxOut)} STYX locked ${F.lockDays * 24}h at ${fmt(r.apy * 100, 0)}% APY` : `bonded ${fmt(r.bonded, 2)} USDG → ${big(r.styxOut)} STYX vesting`);
