...
const W = 1280, H = 720, FPS = 30, DUR = 10, PORT = 9487;
const OUT = path.join(__dirname, '..', 'brand', 'styx-rite3-10s.mp4');
const FRAMES = path.join(__dirname, 'seal-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'seal-video.html').split(path.sep).join('/');
