// THE FORGE hype video — deterministic render: seek each frame, screenshot, encode locked 30fps (zero dropped frames).
'use strict';
...
const W = 1280, H = 720, FPS = 30, DUR = 10, PORT = 9507;
const OUT = path.join(__dirname, '..', 'brand', 'styx-forge-10s.mp4');
const FRAMES = path.join(__dirname, 'forge-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'forge-video.html').split(path.sep).join('/');
