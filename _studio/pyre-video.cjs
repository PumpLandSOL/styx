...
const W = 1280, H = 720, FPS = 30, DUR = 15, PORT = 9483;
const OUT = path.join(__dirname, '..', 'brand', 'styx-pyre-15s.mp4');
const FRAMES = path.join(__dirname, 'pyre-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'pyre-video.html').split(path.sep).join('/');
