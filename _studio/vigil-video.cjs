...
const W = 1280, H = 720, FPS = 30, DUR = 15, PORT = 9485;
const OUT = path.join(__dirname, '..', 'brand', 'styx-vigil-15s.mp4');
const FRAMES = path.join(__dirname, 'vigil-frames');
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
(async () => {
  fs.rmSync(FRAMES, { recursive: true, force: true }); fs.mkdirSync(FRAMES);
  const url = 'file:///' + path.join(__dirname, 'vigil-video.html').split(path.sep).join('/');
